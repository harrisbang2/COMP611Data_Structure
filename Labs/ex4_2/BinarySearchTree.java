...
// check order codes in BinaryTreeIterator() and please use // to check each functions
   ...
   implements SortedSet<E>
{
   ...
   
    /////////////////////           LAB
...
// check order codes in BinaryTreeIterator() and please use // to check each functions
         System.out.println("traversalInOrder");
          //traversalInOrder(rootNode);
         System.out.println("check pre-order in BinaryTreeIterator()");
         preorder(rootNode);
         System.out.println("check post-order in BinaryTreeIterator()");
         //postorder(rootNode);
          System.out.println("check level-order in BinaryTreeIterator()");
         //levelorder(rootNode);
...
// root > left > right
          if (node!=null)
         {  
            if (withinView(node.element)&&!list.contains(node.element))
            list.add(node.element);
    ...
    //  left > right> root 
      if (node!=null)
         {      postorder(node.leftChild);
            ...
            if (withinView(node.element)){
                if(node.leftChild ==null&&node.rightChild==null){ // if it is leaf
                 list.add(node.element);
                }
                // parent but one left child and it is already added
              if(node.leftChild !=null){  
                  if(list.contains(node.rightChild.element)&&node.rightChild==null){
                  list.add(node.element);
                  }
                }
               // after when both children has alread added;
              if(!list.contains(node.element)){
              list.add(node.element);
         }
        }              
         }
        } 
        // root - level 2 children - level 3 children
        private void levelorder(BinaryTreeNode node) {  
         ...
         {  
           ...
           if(list.contains(node.element)){
            ...
            if(node.leftChild!=null){
            list.add(node.leftChild.element);
            }
            if(node.rightChild!=null){
            list.add(node.rightChild.element);
            }
